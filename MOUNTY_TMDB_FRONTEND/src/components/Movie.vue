...
export default {
  ...
  methods: {
    fetchMovie(id){
      axios.get(`https://developers.themoviedb.org/3/movies/get-top-rated-movies${id}?api_key=${storage.apiKey}&language=en-US`)
      .then(function(resp){
          let movie = resp.data;
          this.movie = movie;
          this.poster();
    ...
    poster() {
      if(this.movie.poster_path){
        this.moviePosterSrc = 'https://image.tmdb.org/t/p/' + this.movie.poster_path;
      }
    },
    backdrop(){
      if(this.movie.backdrop_path){
        this.movieBackdropSrc = 'https://image.tmdb.org/t/p/' + this.movie.backdrop_path;
      }
    },
